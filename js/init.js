...
jQuery(document).ready(function(){
	...
	function beny_tm_contact_form(){
		...
		jQuery(".contact_form #send_message").on('click', function(){
			...
			//checking for blank fields and valid email
			if(name===''||email===''||message===''||!emailValid){

				jQuery('div.empty_notice').slideDown(500).delay(2000).slideUp(500);
			}
			else{
				// Send via Web3Forms (no backend needed on static GitHub Pages)
				...
				.then(function(data){
					...
					if(data.success){
						...
						jQuery(".contact_form .returnmessage").append("<span class='contact_error'>Something went wrong. Please try again or email directly.</span>");
						jQuery(".contact_form .returnmessage").slideDown(500).delay(4000).slideUp(500);
					}
				})
				.catch(function(){
					jQuery(".contact_form .returnmessage").empty();
					jQuery(".contact_form .returnmessage").append("<span class='contact_error'>Network error. Please try again later.</span>");
					jQuery(".contact_form .returnmessage").slideDown(500).delay(4000).slideUp(500);
				});
			}
			return false;
